...
const calculateUpcomingShifts = (shifts) => {
  ...
  const shiftsNotInCurrentDuration = shiftsInCurrentWeek.filter((shift) => {
    const shiftStart = new Date(shift.start);

    return now < shiftStart ;
  });

  shiftsNotInCurrentDuration.sort((a, b) => {
    const dateComparison = new Date(a.start) - new Date(b.start);
    return dateComparison !== 0 ? dateComparison : a.day - b.day;
  });

  const nextTwoShifts = shiftsNotInCurrentDuration.slice(0, 2);
...
const setNextWeekSchedule = async (req, res) => {};
